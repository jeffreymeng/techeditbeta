// https://github.com/ts95/lang-detector/blob/master/index.js | MIT License

// Sightly adapted and improved for browser use by Jeffrey Meng
 ...
 *  1 = Regular point:  Part of but not unique to a given language.
 * -1 = Penalty point:  Does not match a given language.
 * Rare:
 * -50 = Bonus penalty points: Only used when two languages are mixed together,
 *  and one has a higher precedence over the other one. (e.g. JavaScript in HTML)
 */
var languages = {
	"JavaScript": [
		// undefined keyword
		...
		// Function call: hi() or $().x(); or  obj.x();
		{ pattern: /^([A-Za-z$_])([\w$]+)?(\((.*)\))?(\.([A-Za-z$_])([\w$]+)?(\((.*)\))?)?/, points: 1 },
